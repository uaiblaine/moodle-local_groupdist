...
| [`mockups/step2-preview-empty.html`](mockups/step2-preview-empty.html) | Step 2 — preview when the run **would write nothing** (2026-08-21). One switcher per reason the builder can report: `nocandidates`, `noroom`, `allplaced`, `nogroups`, plus a control that writes. Each of these used to render as a row of zeros, a grid of "0 new members" cards and a greyed Apply with no sentence anywhere. The sample grid is retired only under `nocandidates` and `nogroups`; under the other two the cards are the evidence and stay |
